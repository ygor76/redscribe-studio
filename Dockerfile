...
# RedScribe Publish Hub 5.3.0 - Smart AI Hub + Studio + TikTok
...
# Ponte híbrida + publicação TikTok + Studio + Smart AI Hub.
...
COPY patches/integrations_hub.js /app/static/integrations_hub.js
...
RUN python -c "from pathlib import Path; p=Path('/app/templates/dashboard.html'); s=p.read_text(encoding='utf-8'); marker='<script src=\"/static/dashboard.js?v=5.0.5\"></script>'; bridge='<script src=\"/static/local_bridge.js?v=5.3.0\"></script>'; s=s if 'local_bridge.js' in s else s.replace(marker, bridge+'\\n'+marker); p.write_text(s, encoding='utf-8')"
...
RUN python -c "from pathlib import Path; p=Path('/app/templates/dashboard.html'); s=p.read_text(encoding='utf-8'); marker='</body>'; patch='<script src=\"/static/studio_finish_patch.js?v=5.2.8\"></script>\\n<script src=\"/static/integrations_hub.js?v=5.3.0\"></script>'; s=s if 'integrations_hub.js' in s else s.replace(marker, patch+'\\n'+marker); p.write_text(s, encoding='utf-8')"
